...
def read_cpp_analysis(fname):
    ...
    try:
        ...
        # Not all IDL files are built by the build system. If an IDL
        # file isn't built, there won't be a header file for it. Don't
...
# This function looks for any definition of something of the form
# "E_..._CONSTANT" for IDL constant CONSTANT. It assumes this is a
# match. Technically, if you have two classes defined in the same IDL
# file, and they use constants with the same name, it will get
# confused. Oh well.
...
def handle_interface(methods, enums, iface):
    ...
    source(lineno, colno, 'class', iface.name,
           [mangled] + if_true(iface.attributes.scriptable, js_mangled_name(iface.name)))
    ...
    if iface.attributes.scriptable:
        target(lineno, colno, 'class', iface.name, js_mangled_name(iface.name))
    ...
    if iface.base:
        ...
        source(lineno, colno, 'class', iface.base,
               [mangled] + if_true(iface.attributes.scriptable, js_mangled_name(iface.base)))

        target(lineno, colno, 'class', iface.base, mangled)
        if iface.attributes.scriptable:
            target(lineno, colno, 'class', iface.base, js_mangled_name(iface.base))
    ...
    for m in iface.members:
        ...
        if isinstance(m, xpidl.Method):
            ...
            source(lineno, colno, 'method', m.name,
                   [mangled] + if_true(not m.noscript, js_mangled_name(m.name)))
            ...
            # JS target
            if not m.noscript:
                target(lineno, colno, 'method', m.name, js_mangled_name(m.name))
            ...
            source(lineno, colno, 'attribute', m.name,
                   ...
                   if_true(not m.noscript, js_mangled_name(m.name)))

            target(lineno, colno, 'attribute', m.name, mangled_getter)
            if not m.readonly:
                target(lineno, colno, 'attribute', m.name, mangled_setter)
            if not m.noscript:
                target(lineno, colno, 'attribute', m.name, js_mangled_name(m.name))

        elif isinstance(m, xpidl.ConstMember):
            mangled = find_enum(enums, m.name)

            # JS source
            source(lineno, colno, 'constant', m.name, [js_mangled_name(m.name)] + if_true(mangled, mangled))

            # JS target
            target(lineno, colno, 'constant', m.name, js_mangled_name(m.name))
